...
from analysis.src.plot_functions import plot_noise_reg
...
def main(
    ...
    Args:
        plot_parameters (PlotParams): Configuration for visualization and result plotting (unused)
    ...
    exp_folder = Path(f"data/experiment_1/T_{experiment.T_for_reg}_E_{experiment.E_for_reg}_nu_{experiment.nu_for_reg}")
    img_indices = sorted(int(f.stem.replace("_img", "")) for f in exp_folder.glob("*_img.npy"))
    
    mu_reg, lambda_reg = compute_lame(experiment.E_for_reg, experiment.nu_for_reg)
    ...
    for imgind in img_indices:  
        rmse_dict_reg = process_test_reg(
            ...
            factors_for_reg=experiment.factors_for_reg
        )
    ...
    plot_noise_reg(rmse_mean_reg, experiment.factors_for_reg, rmse_mean_noise, stds, Path(general.results_dir))
