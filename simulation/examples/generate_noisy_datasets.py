...
import numpy as np
import jsonargparse
from simulation.src.config import ExperimentsParams
from simulation.src.simulation_pipeline import create_noisy_images
...
def main(
    ...
    Args:
        exp (ExperimentsParams): Experiment parameters, including:
              - `T_for_noisy_exp`: Traction force to locate the original dataset.
              - `E_for_noisy_exp`: Young’s modulus to locate the original dataset.
              - `nu_for_noisy_exp`: Poisson’s ratio to locate the original dataset.
    ...
    """
    dataset_root = "data"
    noise_output_root = f"data/noise_experiment_T_{exp.T_for_noisy_exp}_E_{exp.E_for_noisy_exp}_nu_{exp.nu_for_noisy_exp}"
    ...
    for exp_name in exp_candidates:
        candidate_dir = os.path.join(dataset_root, f"{exp_name}/T_{exp.T_for_noisy_exp}_E_{exp.E_for_noisy_exp}_nu_{exp.nu_for_noisy_exp}")
        if os.path.exists(candidate_dir):
            input_dir = candidate_dir
            break
    if input_dir is None:
        raise FileNotFoundError(f"Requested T={exp.T_for_noisy_exp}, E={exp.E_for_noisy_exp}, nu={exp.nu_for_noisy_exp} not found in any experiment.")
    ...
    for seed in exp.seeds:
        ...
        noisy_images, disps = create_noisy_images(img, disp, exp.noise_stds, seed)
        print("Noisy images done !")
        for std, noisy_img, disp_gt in zip(exp.noise_stds, noisy_images, disps):
